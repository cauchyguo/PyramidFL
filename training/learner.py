...
def report_data_info(rank, queue):
    ...
    clientIdToRun = torch.zeros(
        [world_size - 1], dtype=torch.int).to(device=device)
    ...
    for file in files:
        if not os.path.isdir(file):
            if '.pth.tar' in file and args.model in file:
                model_state_id = int(re.findall(
                    args.model+"_(.+?).pth.tar", file)[0])
...
def run_client(idx, clientId, cmodel, iters, learning_rate, dropout_ratio, argdicts={}):
    ...
    if args.task != 'nlp' and args.task != 'text_clf' and args.task != 'har':
        optimizer = MySGD(cmodel.parameters(), lr=learning_rate,
        ...
        optimizer_grouped_parameters = [
            ...
            {"params": [p for n, p in cmodel.named_parameters() if any(
                nd in n for nd in no_decay)], "weight_decay": 0.0},
        ]
        optimizer = AdamW(optimizer_grouped_parameters,
                          lr=learning_rate, eps=args.adam_epsilon, weight_decay=5e-4)
    ...
    if clientId not in global_data_iter:
        ...
        global_data_iter[clientId] = [train_data_itr,
                                      curBatch, total_batch_size, argdicts['iters']]
    else:
        [train_data_itr, curBatch, total_batch_size,
            epo] = global_data_iter[clientId]
    ...
    try:
        for itr in range(iters):
                ...
                try:
                    try:
                        if args.task == 'nlp':
                            ...
                            (data, masks), target = next(
                                train_data_itr_list[0])
                            masks = Variable(masks).to(device=device)
                        elif args.task == 'voice':
                            (data, target, input_percentages, target_sizes), _ = next(
                                train_data_itr_list[0])
            ...
            if args.task == 'nlp':
                outputs = cmodel(data, masked_lm_labels=target) if args.mlm else cmodel(
                    data, labels=target)
                loss = outputs[0]
                # torch.nn.utils.clip_grad_norm_(cmodel.parameters(), args.max_grad_norm)
            elif args.task == 'text_clf':
                loss, logits = cmodel(
                    data, token_type_ids=None, attention_mask=masks, labels=target)
            ...
            if itr < total_batch_size:  # >= (iters - total_batch_size - 1):
                if epoch_train_loss is None:
                    ...
                    epoch_train_loss = (1. - args.loss_decay) * \
                        epoch_train_loss + args.loss_decay * temp_loss
            ...
            if args.task != 'nlp' and args.task != 'text_clf' and args.task != 'har':
                if args.enable_obs_importance:
                    for id_loss_item, loss_item in enumerate(loss):
                        ...
                        delta_w_persample, gradient_l2_norm_persample = optimizer.get_delta_importance_sampling(
                            learning_rate)

                        importance_matrix[id_loss_item,
                                          itr, 0] = float(loss_item.data)
                        importance_matrix[id_loss_item, itr, 1] = float(
                            gradient_l2_norm_persample)
                    timecost_matrix[itr,
                                    2] += round(time.time() - update_start, 4)
                ...
                if (not args.proxy_avg):
                    ...
                    for idx, param in enumerate(cmodel.parameters()):
                        param.data -= delta_w[idx].to(device=device)
                        param.data += learning_rate * args.proxy_mu * \
                            (last_model_tensors[idx] - param.data)
            else:
                # proxy term
                optimizer.zero_grad()
                loss.mean().backward()
            ...
            if args.enable_obs_importance:
                ...
                timecost_matrix[itr, 1] += timecost_matrix[itr, 0] - \
                    timecost_matrix[itr, 2]-timecost_matrix[itr, 3]
        if args.enable_obs_importance:
        ...
        if len(global_client_profile) > args.max_iter_store:
            ...
            rmClient = sorted(
                allClients, key=lambda k: global_data_iter[k][3])[0]
    ...
    if total_batch_size > iters * 10 and len(train_data_itr_list) > 0 and not args.release_cache:
        global_data_iter[clientId] = [train_data_itr_list[0],
                                      curBatch, total_batch_size, argdicts['iters']]
    else:
        if args.num_loaders > 0:
    ...
    model_param = [(param.data - last_model_tensors[idx]).cpu().numpy()*(random.uniform(0, 1)
                                                                         >= dropout_ratio) for idx, param in enumerate(cmodel.parameters())]
    ...
    if clientId in global_client_profile:
        time_cost = global_client_profile[clientId][0] * \
            count + global_client_profile[clientId][1]
    else:
...
def run(rank, model, queue, param_q, stop_flag, client_cfg):
    ...
    for epoch in range(args.load_epoch, int(args.epochs) + args.load_epoch):
        try:
            ...
            learning_rate = max(args.min_learning_rate, args.learning_rate *
                                (args.decay_factor**((epoch) // args.decay_epoch)))
            ...
            if not args.test_only:
                ...
                for idx, nextClientId in enumerate(nextClientIds):
                    ...
                    LocalEpochRation = 1 if nextClientLocalEpoch == None or not args.enable_adapt_local_epoch else nextClientLocalEpoch[
                        idx]

                    LocalDropoutRatio = 0 if nextClientDropoutRatio == None or not args.enable_dropout else nextClientDropoutRatio[
                        idx]
                    _model_param, _loss, _trained_size, _speed, _time, _isSuccess = run_client(idx=idx,
                                                                                               ...
                                                                                               iters=int(
                                                                                                   args.upload_epoch*LocalEpochRation),
                    ...
                    if args.forward_pass:
                        forward_dataset = select_dataset(
                            nextClientId, global_trainDB, batch_size=args.test_bsz, isTest=True)
                ...
                for nextClientId in nextClientIds:
                    ...
                    test_loss, acc, acc_5, temp_testResults = test_model(
                        rank, model, client_dataset, criterion=criterion, tokenizer=tokenizer)

                    logging.info(
                        f'====Epoch: {epoch}, clientId: {nextClientId}, len: {temp_testResults[-1]}, test_loss: {test_loss}, acc: {acc}, acc_5: {acc_5}')
            ...
            queue.put({rank: [trainedModels, preTrainedLoss, trainedSize,
                      isComplete, ranClients, trainSpeed, testResults, virtualClock]})
            uploadEpoch = -1
            ...
            step_tensor = torch.zeros(
                [world_size], dtype=torch.int).to(device=device)
            ...
            clients_tensor = torch.zeros(
                [totalLen], dtype=torch.int).to(device=device)
            dist.broadcast(tensor=clients_tensor, src=0)

            clients_local_epoch_tensor = torch.zeros(
                [totalLen], dtype=torch.float).to(device=device)
            dist.broadcast(tensor=clients_local_epoch_tensor, src=0)

            clients_dropout_ratio_tensor = torch.zeros(
                [totalLen], dtype=torch.float).to(device=device)
            dist.broadcast(tensor=clients_dropout_ratio_tensor, src=0)

            nextClientIds = [clients_tensor[x].item()
                             for x in range(startIdx, endIdx)]
            nextClientLocalEpoch = [
                clients_local_epoch_tensor[x].item() for x in range(startIdx, endIdx)]
            nextClientDropoutRatio = [
                clients_dropout_ratio_tensor[x].item() for x in range(startIdx, endIdx)]
            ...
            if epoch % int(args.eval_interval) == 0:
                ...
                if args.test_train_data:
                    ...
                    test_loss, acc, acc_5, testResults = test_model(
                        rank, model, rank_train_data, criterion=criterion, tokenizer=tokenizer)
                else:
                    test_loss, acc, acc_5, testResults = test_model(
                        rank, model, global_testDB, criterion=criterion, tokenizer=tokenizer)
            ...
            if epoch % (args.epochs-1) == 0 and args.this_rank == 1:
                modelPath = os.path.join(args.log_path, 'logs', args.job_name, args.time_stamp, str(
                    args.model)+'_'+str(epoch)+'.pth.tar')
    ...
    logging.info("Worker {} has completed epoch {}!".format(
        args.this_rank, epoch))
...
if __name__ == "__main__":
    ...
    dist.init_process_group(
        args.backend, rank=this_rank, world_size=world_size)
    ...
    dataConf = os.path.join(
        args.data_dir, 'sampleConf') if args.data_set == 'imagenet' else None
    ...
    testsetPartitioner = DataPartitioner(
        data=test_dataset, isTest=True, numOfClass=args.num_class)
    ...
    partition_dataset(testsetPartitioner, [
                      i for i in range(world_size-1)], splitTestRatio)
    global_testDB = select_dataset(
        this_rank, testsetPartitioner, batch_size=args.test_bsz, isTest=True, collate_fn=collate_fn)
